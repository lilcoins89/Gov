...
export function Navbar() {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-20 border-b border-white/10 bg-navy-950/10 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
        ...
        <nav className={`${open ? 'flex' : 'hidden'} absolute left-4 right-4 top-[4.5rem] flex-col gap-1 rounded-2xl border border-navy-700 bg-navy-900 p-3 shadow-xl md:static md:flex md:flex-row md:items-center md:gap-7 md:border-0 md:bg-transparent md:p-0 md:shadow-none`} data-testid="nav-public">
          ...
          <Link href="/login" onClick={() => setOpen(false)} className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg border border-teal-300/45 bg-white/[0.04] px-4 py-2.5 text-sm font-bold text-white transition hover:border-teal-300 hover:bg-teal-300 hover:text-navy-950 md:mt-0">Client sign in <ArrowRight size={15} /></Link>
        </nav>
