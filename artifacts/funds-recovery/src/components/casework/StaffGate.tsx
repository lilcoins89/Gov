...
function hasStaffAccess() {
  try {
    return sessionStorage.getItem(STAFF_ACCESS_KEY) === 'granted';
  } catch {
    return false;
  }
}
...
export function StaffGate({ children }: { children: ReactNode }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    try { sessionStorage.setItem(STAFF_ACCESS_KEY, 'granted'); } catch { /* local-only demo */ }
    setGranted(true);
    setError('');
  };
    ...
    <main className="grid min-h-dvh bg-background lg:grid-cols-[.8fr_1.2fr]">
      ...
      <section className="flex flex-col px-5 py-7 sm:px-10 lg:justify-center lg:px-20">
        <div className="mx-auto w-full max-w-md">
          ...
          <div className="mt-16 lg:mt-0"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-teal-50 text-teal-800"><LockKeyhole size={22} /></div><p className="mt-8 text-xs font-bold uppercase tracking-[0.2em] text-teal-800">Staff access required</p><h2 className="mt-4 font-display text-4xl font-extrabold tracking-[-0.045em] text-navy-900">Review desk sign in</h2><p className="mt-3 leading-7 text-muted-foreground">This internal area is not linked from the public site. Enter your staff access code to continue.</p></div>
          ...
          <p className="mt-8 text-center text-xs leading-5 text-muted-foreground">This is a local demo gate. Connect it to your production identity provider before exposing staff data.</p>
        </div>
      </section>
    </main>
  );
